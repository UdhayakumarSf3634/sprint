...
function App(this: any) {
  ...
  let status: any;
  let gantt = React.useRef<any>();
let kanbanObj = React.useRef<any>();
let gridObj = React.useRef<any>();

  let scheduleObj = React.useRef<any>();
  let topDropDownInstance = React.useRef<any>();
  let dateRangeInstance = React.useRef<any>();
  ...
  function resourceFilterImage(value: any): void {
    let projectValue;
    if (topDropDownInstance.current !== null && typeof topDropDownInstance.current !== 'undefined') {
      const element = topDropDownInstance.current as HTMLInputElement;
      projectValue = element.value;
    }
    let dateRangeValue: any;
    if (dateRangeInstance.current !== null && typeof dateRangeInstance.current !== 'undefined') {
      const element = dateRangeInstance.current as HTMLInputElement;
      dateRangeValue = element.value;
    }
  ...
  function bindTabClickEvent(): void {
    ...
    tabInfo.forEach(tab => {
      ...
      Array.from(parentElements).forEach((element: Element) => {
        element.addEventListener("click", () => {
          setActiveTab(element as HTMLElement); // Type assertion
        });
      });
    });
  ...
  function bindEventListeners(): void {
    ...
    Array.from(parentElements.children).forEach((element: Element) => {
      const htmlElement = element as HTMLElement;
      htmlElement.addEventListener('click', () => {
        setActiveTab(htmlElement);
      });
    });
